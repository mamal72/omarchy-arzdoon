...
GitHub Actions runs the test suite on Python 3.12, 3.13, and 3.14, checks Python formatting and types, validates the manifest, and parses both QML entry points. Pushing a version tag such as `v1.0.2` publishes a GitHub release after every check passes; the tag must match the version in `manifest.json`.
